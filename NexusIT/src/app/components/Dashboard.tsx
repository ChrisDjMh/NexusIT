...
export function Dashboard({ onTicketSelect, onNavegar, usuario }: DashboardProps) {
  ...
  useEffect(() => {
    const cargarDatos = async () => {
      try {
        // 2. CORREGIDO: Se reemplazaron las URLs estáticas por la variable centralizada
  ...
  return (
    <div className="flex-1 overflow-y-auto p-5 space-y-5">
      <div className="grid gap-4" style={{ gridTemplateColumns: 'repeat(4, 1fr)' }}>
        <TarjetaKPI icon={Flame} etiqueta="Crítico Abierto" valor={criticos.length} detalle="Requiere atención inmediata" color="#ef4444" />
        <TarjetaKPI icon={Zap} etiqueta="En Progreso" valor={enProgreso.length} detalle="Activos este sprint" color="var(--color-cyber-blue)" />
        <TarjetaKPI icon={CheckCircle2} etiqueta="Resueltos" valor={resueltos.length} detalle="Total completados" color="#10b981" />
        <TarjetaKPI icon={AlertTriangle} etiqueta="Vencidos" valor={vencidos.length} detalle="Pasaron la fecha límite" color="#f59e0b" />
      </div>
      ...
      <div className="grid gap-4" style={{ gridTemplateColumns: '1.4fr 1fr' }}>
        <div className="rounded-xl border overflow-hidden" style={{ background: 'var(--card)', borderColor: 'var(--border)' }}>
          <div className="px-5 py-3.5 border-b flex items-center justify-between" style={{ borderColor: 'var(--border)' }}>
            <h3 style={{ fontSize: 13, fontWeight: 600, color: 'var(--foreground)' }}>Prioridad Crítica y Alta</h3>
            <button onClick={() => onNavegar('tickets')} className="flex items-center gap-1 hover:opacity-70 transition-opacity" style={{ fontSize: 11, color: 'var(--color-cyber-blue)' }}>
              Ver todos <ArrowRight size={11} />
            </button>
          </div>
          <div>
            {ticketsAltosPriority.slice(0, 5).map((t, i, arr) => (
              <button key={t.id} onClick={() => onTicketSelect(t)} className="w-full flex items-center gap-3 px-5 py-3 hover:bg-[var(--muted)] transition-colors text-left" style={{ borderBottom: i < arr.length - 1 ? '1px solid var(--border)' : 'none' }}>
                <span className="w-2 h-2 rounded-full flex-shrink-0" style={{ background: t.prioridad === 'Crítico' ? '#ef4444' : '#f97316' }} />
                ...
                <div className="w-6 h-6 rounded-full flex items-center justify-center flex-shrink-0" style={{ background: t.asignado.color, fontSize: 9, color: '#fff', fontWeight: 700 }} title={t.asignado.nombre}>
                  {t.asignado.iniciales}
                </div>
              </button>
            ))}
            {ticketsAltosPriority.length === 0 && (
              <div className="p-5 text-center" style={{ fontSize: 12, color: 'var(--muted-foreground)' }}>No hay tickets críticos o altos activos.</div>
            )}
          </div>
        </div>
        ...
        <div className="rounded-xl border overflow-hidden" style={{ background: 'var(--card)', borderColor: 'var(--border)' }}>
          <div className="px-5 py-3.5 border-b flex items-center justify-between" style={{ borderColor: 'var(--border)' }}>
            <h3 style={{ fontSize: 13, fontWeight: 600, color: 'var(--foreground)' }}>Proyectos Activos</h3>
            <button onClick={() => onNavegar('cronograma')} className="flex items-center gap-1 hover:opacity-70 transition-opacity" style={{ fontSize: 11, color: 'var(--color-cyber-blue)' }}>
          ...
          <div className="p-5 flex flex-col gap-4">
            ...
            {proyectos.length === 0 && (
              <div className="text-center" style={{ fontSize: 12, color: 'var(--muted-foreground)' }}>No hay proyectos registrados.</div>
            )}
          </div>
        </div>
      </div>
      ...
      <div className="rounded-xl border overflow-hidden" style={{ background: 'var(--card)', borderColor: 'var(--border)' }}>
        ...
        <div>
          {actividadReciente.map((log, i, arr) => (
            <button key={log.id} onClick={() => onTicketSelect(log.ticket)} className="w-full flex items-center gap-3 px-5 py-3 hover:bg-[var(--muted)] transition-colors text-left" style={{ borderBottom: i < arr.length - 1 ? '1px solid var(--border)' : 'none' }}>
              <div className="w-6 h-6 rounded-full flex items-center justify-center flex-shrink-0" style={{ background: colorPorIniciales(log.iniciales), fontSize: 9, color: '#fff', fontWeight: 700 }}>
                {log.iniciales}
              </div>
              <div className="flex-1 min-w-0">
                <span style={{ fontSize: 12, color: 'var(--foreground)' }}>
                  <span style={{ fontWeight: 600 }}>{log.usuario}</span>{' '}{accionEtiqueta(log.tipo)}{' '}<span className="mono" style={{ color: 'var(--color-cyber-blue)', fontSize: 11 }}>{log.ticketId}</span>
                </span>
                <div style={{ fontSize: 11, color: 'var(--muted-foreground)' }} className="truncate">{log.accion.replace(/\*\ Third/g, '')}</div>
              </div>
              <span style={{ fontSize: 10, color: 'var(--muted-foreground)', flexShrink: 0 }}>{log.tiempo}</span>
            </button>
          ))}
          {actividadReciente.length === 0 && (
            <div className="p-5 text-center" style={{ fontSize: 12, color: 'var(--muted-foreground)' }}>No hay actividad reciente registrada.</div>
          )}
        </div>
      </div>
    </div>
...
function TarjetaKPI({ icon: Icon, etiqueta, valor, detalle, color }: { icon: React.ElementType; etiqueta: string; valor: number; detalle: string; color: string; }) {
  return (
...
function colorPorIniciales(iniciales: string) {
  const mapa: Record<string, string> = { VL: '#3b82f6', CH: '#8b5cf6', BR: '#10b981', FE: '#f59e0b', DK: '#ef4444' };
  return mapa[iniciales] || '#6b7280';
}

function accionEtiqueta(tipo: string) {
  const mapa: Record<string, string> = { comentario: 'comentó en', estado: 'cambió el estado de', asignacion: 'fue asignado a', commit: 'vinculó un commit a' };
